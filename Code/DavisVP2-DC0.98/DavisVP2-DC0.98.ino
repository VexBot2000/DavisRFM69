...
struct radioDATARAW
{
  uint8_t DATA_Wind[DAVIS_PACKET_LEN];
  uint8_t DATA_WindDir[DAVIS_PACKET_LEN];
  uint8_t DATA_Temp[DAVIS_PACKET_LEN];
  uint8_t DATA_Hum[DAVIS_PACKET_LEN];
  uint8_t DATA_Rain[DAVIS_PACKET_LEN];
  uint8_t DATA_Solar[DAVIS_PACKET_LEN];
  uint8_t DATA_UV[DAVIS_PACKET_LEN];
  uint8_t DATA_Extra[DAVIS_PACKET_LEN];
};
...
void setup()
{
   wdt_enable(WDTO_8S); // Enable the watchdog timer with a 2 second timeout
  ...
  sCmd.addCommand("READLOOP", readloopData);
  sCmd.addCommand("READSAVED", readsavedData);
  sCmd.addCommand("READRAW", readsavedDataRAW);
...
void loop()
{
  ...
  if (Serial.available() > 0)
    loopCount = 0; // if we receive anything while sending LOOP packets, stop the stream
  sendLoopPacket();
...
void processPacket()
{
 ...
 memcpy((void*)radioDATAraw.DATA_WindDir, (const void*)radio.DATA,DAVIS_PACKET_LEN);
  memcpy((void*)radioDATAraw.DATA_Wind, (const void*)radio.DATA,DAVIS_PACKET_LEN);
  ...
  {
    memcpy((void*)radioDATAraw.DATA_Hum, (const void*)radio.DATA, DAVIS_PACKET_LEN);
  ...
  {
    memcpy((void*)radioDATAraw.DATA_Rain, (const void*)radio.DATA, DAVIS_PACKET_LEN);
  ...
  // DANE - INDEX UV
  case VP2P_UV:
    memcpy((void*)radioDATAraw.DATA_UV, (const void*)radio.DATA,DAVIS_PACKET_LEN);
  ...
  case VP2P_SOLAR:
  memcpy((void*)radioDATAraw.DATA_Solar, (const void*)radio.DATA,  DAVIS_PACKET_LEN);
...
void cmdReadRegs()
{ // Opcja debugowania wyświetla wszystkie rejestry moteino
...
void sendLoopPacket()
{
  if (loopCount <= 0 || millis() - lastLoopTime < LOOP_INTERVAL)
    return;
  lastLoopTime = millis();
  loopCount--;
  ...
  for (uint8_t i = 0; i < sizeof(loopData); i++)
    Serial.write(loopPtr[i]);
  Serial.write(highByte(crc));
  Serial.write(lowByte(crc));
}
...
void cmdDmpaft()
{
  ...
  while (Serial.available() <= 0)
    ;
  if (Serial.read() != 0x06)
    ; // should return if condition is true, but can't get a 0x06 here for the life of me, even if weewx does send it...
  ...
  for (uint8_t i = 0; i < 5; i++)
  {
    ...
    for (uint8_t j = 0; j < sizeof(fakeArchiveRec); j++)
      Serial.write(farp[j]);
  }
  for (uint8_t i = 0; i < 4; i++)
    Serial.write(0);
...
void help()
{
  ...
  Serial.println(F("spis komend:"));

  Serial.println(F("RESET"));
  Serial.println(F("LOOP - Send the loop data"));
  Serial.println(F("STRMOFF - Disable printing of received packet data"));
  Serial.println(F("STRMON - Enable printing of received packet data"));
  Serial.println(F("TEST - Echo's TEST"));
  Serial.println(F("INFO - Analizuje ostatnio odebrany pakiet"));
  Serial.println(F("ALLREG - wyswietla wszytskie stany rejestrow"));
  Serial.println(F("HELP - wyswietla ten komunikat"));
  Serial.println(F("READLOOP - wyswietla ostatnio odebrane dane"));
  Serial.println(F("READSAVED - wyswietla ostatnio zapisane dane"));
  Serial.println(F("READRAW - wyswietla ostatnio odebrane dane w formacie hex"));

  // Oznaczenia danych: ws - predkosc wiatru [mile/h],wd - kierunek wiatru w stopniach,bat - stan baterii (1 brak lub rozładowana,a 0 naładowana)
...
void readloopData(uint8_t radioDATAraw[10])
{
  for (uint8_t i = 0; i < DAVIS_PACKET_LEN; i++)
  {
    Serial.print(i);
    Serial.print(" = ");
    String dane = String(radioDATAraw[i], HEX);
    dane.toLowerCase();
    Serial.println(dane);
  }
  Serial.println();
...
void readsavedDataRAW(){
  Serial.println(F("READRAW "));
  Serial.println(F(" ws"));
  if (radioDATAraw.DATA_Extra != 0)
  {
    readloopData(radioDATAraw.DATA_Wind);
  
  }
  Serial.println(F(" wd"));
  if (radioDATAraw.DATA_Extra != 0)
  {
    readloopData(radioDATAraw.DATA_WindDir);
  }
 Serial.println(F(" ta"));
  if (radioDATAraw.DATA_Temp != 0)
  {
    readloopData(radioDATAraw.DATA_Temp);
  }
  Serial.println(F(" rh"));
  if (radioDATAraw.DATA_Hum != 0)
  {
    readloopData(radioDATAraw.DATA_Hum);
  }
  Serial.println(F(" re"));
  if (radioDATAraw.DATA_Rain != 0)
  {
    readloopData(radioDATAraw.DATA_Rain);
  }
  Serial.println(F(" sr"));
  if (radioDATAraw.DATA_Solar != 0)
  {
    readloopData(radioDATAraw.DATA_Solar);
  }
  Serial.println(F(" uv"));
  if (radioDATAraw.DATA_UV != 0)
  {
    readloopData(radioDATAraw.DATA_UV);
  }
...
void readsavedData()
{
  Serial.print(F("READSAVED "));
  if (loopData.windSpeed != 0)
  {
    Serial.print(" ws ");
    Serial.print(loopData.windSpeed);
  }
  ...
  if (loopData.transmitterBatteryStatus != 0)
  {
    Serial.print(F(" bat "));
    Serial.print(loopData.transmitterBatteryStatus);
  }
  ...
  if (loopData.outsideTemperature != 0)
  {
    Serial.print(F(" ta "));
